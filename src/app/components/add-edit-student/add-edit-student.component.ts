...
export class AddEditStudentComponent  implements OnInit{
  ...
  constructor(private fb: FormBuilder,
    ...
    this.addStudent = this.fb.group({
      ...
      sex: ['', [Validators.required, Validators.maxLength(1),Validators.minLength(1)]],
  ...
  addEditStudent(){
    if(this.dtoStudent == undefined){
      ...
      this._studentService.saveStudent(formData).subscribe(data =>{
        this.toastr.success('El estudiante fue registrado con exito', 'Registro completo!');
      ...
      //Editamos Student
        ...
        formData.append("dtoStudent.phone",this.addStudent.get('phone')?.value);
        console.log(formData);
        this._studentService.updateStudent(formData).subscribe(data =>{
        this.toastr.info('El estudiante fue actualizado con exito', 'Estudiante actualizado!');
        this.router.navigate(['/getall']);
      }, error =>{
        this.toastr.error('Opss ocurrio un error', 'Error');
        console.log(error);
